...
export const getAllTodo = async (req: RequestWithUserRole, res: Response) => {
  ...
  data.forEach(async(todo)=>{
    if((todo.dueDate && new Date(todo.dueDate))<currDate && !todo.isOverdue){
        todo.isOverdue=true,
...
export const filterTodo=async(req:RequestWithUserRole,res:Response)=>{
    ...
    if(!filter&&filter!='ACTIVE' && filter!='COMPLETED'&&filter!='ALL'){
        return res.status(404).json({
            success:false,
...
export const filterPriority=async(req:RequestWithUserRole,res:Response)=>{
   try {
     const filter=req.params.filId 
    if(!filter&&filter!='LOW' && filter!='MEDIUM'&&filter!='HIGH'){
        return res.status(404).json({
            success:false,
            message:"wrong filter option"
        })
